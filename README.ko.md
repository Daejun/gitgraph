...
| `ime_switch` | `GITGRAPH_IME_SWITCH` | `true` | TUI: 이 머신에 fcitx5가 있으면 gg가 키보드를 잡는 순간(시작, `C`에서 복귀, 텍스트 prompt 종료)에 IME를 영문으로 바꾸고 종료 때 되돌림. 로컬 `fcitx5-remote`가 없으면(ssh, PuTTY) 아무 것도 하지 않음 |
...
| 한글 IME | 이 머신에 fcitx5가 있으면 gg가 키보드를 잡을 때 — 시작, `C`에서 복귀, 텍스트 prompt가 닫힐 때(한글로 쓴 prompt는 다음에도 한글로 열림) — IME를 직접 영문으로 바꾸고 종료 때 되돌림(`ime_switch`). 그 외 환경에서는 한글 모드에서도 단축키가 한 키 늦게 먹음(IME가 다음 키까지 글자를 붙들고 있어서): 들어온 자모·음절을 2벌식 자판 키로 되돌림(`ㅓ` = j, `ㅏ` = k, `자` = w k) |
...
| 패널 | 내용 | Enter |
|---|---|---|
| 1 Files | PR 한 줄, 그 아래 변경 파일마다 `+추가 -삭제`와 그 파일에 걸린 가장 무거운 지적(`⚠` 결함 — reach/bug/regression/logic, `ℹ` 의견 — style/design). 맨 아래 줄은 worktree가 디스크에서 차지하는 용량 | 그 파일의 diff를 Diff에 띄운다 |
| 2 Diff | 그 파일의 unified diff, 컨텍스트 5줄, hunk 접기 가능. gutter는 새 파일 기준 줄 번호(삭제된 줄은 옛 번호)와 그 줄에 지적이 있을 때의 표시. 탭은 4칸으로 편다 | hunk 접기/펴기 |
| 3 Findings | 탭은 한 번에 하나(`[` `]`, `‹ open ›`으로 표시): **open** · **posted** · **ignored** · **dropped**(반증된 것) · **changes**(리뷰가 diff를 어떻게 쪼갰는지, 바뀐 코드가 실행되기는 하는지) · **github**(PR에 이미 달린 리뷰 스레드 — 사람이 한 말을 반복하지 않도록). 지적마다 판정(`✓` 확정, `?` 그럴듯함, `review_verify off`로 돈 리뷰는 `-` — 그때 현재 탭은 `‹ open · unverified ›`로 읽힌다)과 파일·줄. GitHub은 hunk 안이면 context 줄에도(새 파일 쪽), 삭제된 줄에도(옛 파일 쪽) 코멘트를 받는다 — hunk 밖의 지적만 가장 가까운 그런 줄로 당겨지고, `1093→1094`로 표시되며, 게시되는 코멘트는 지적이 실제로 가리키는 곳을 첫머리에 밝힌다 | Diff를 그 줄로 옮긴다 |
...
| 그래프 키(`a` `m` `b` `f` `u` …) | 메시지와 함께 거부된다 — 이 화면 뒤의 그래프에 작용하는 키들이다. `v`로 그래프에 돌아가서 쓰라 |
| `R` · `r` | 리뷰를 돌린다(먼저 물어본다) · PR과 diff만 다시 읽는다(캐시된 지적은 유지) |
| `d` · `i` | Findings 커서가 놓인 것의 전문을 읽는다 — 지적(본문·근거·제안 diff), changes의 한 항목, GitHub 스레드 · 같은 것을 `lang`으로: 번역은 백그라운드로 돌고 끝나면 팝업이 스스로 열린다. `P`가 올리는 것은 언제나 원문이다 |
...
도는 동안 하단 줄이 모델이 지금 하는 일을 그대로 보여준다 — `Read gc.c · 14 tool calls`. 리뷰는 도구 호출 수십 번짜리 에이전트 세션이라, 그게 지나가는 걸 보는 것과 멈춘 카운터를 보는 것은 기다림의 질이 다르다.
