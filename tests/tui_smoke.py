...
def main():
    ...
    check("panels drawn", s.wait_for("5 People") and "0 Main" in s.line(0))
    s.key("j")
    s.key("\r", 2.0)
    check("Enter sets the item (Repo shows it, main holds)", "item: " in s.line(2) and "⊙ hold" in s.line(0))
    s.key("3")
    s.key("j")
    s.key("\r", 2.0)
    item_after_link = s.line(2)
    s.key("b", 2.0)
    check("Links Enter then back", item_after_link != s.line(2))
    s.key("0")
    s.key("]", 1.0)
    check("main tree tab", "[tree]" in s.line(0) and "▾" in s.text())
    s.key("+", 0.8)
    ...
    bx = next((i for i, ch in enumerate(s.line(6)) if ch == "╮"), None)   # Home's top border
    if bx is not None:
        s.mouse(0, bx, 10)
        s.mouse(32, bx + 8, 10)
        s.mouse(32, bx + 15, 10)
        ...
        s.drain(0.8)
        nbx = next((i for i, ch in enumerate(s.line(6)) if ch == "╮"), None)
