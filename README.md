...
| 3 Home | one section at a time (`[` `]`): my turn · todo · mentions · opened · active · waiting · mine · PRs by others · stale · all — same rules as before ("my turn" = items I am in where someone else spoke last, `--days N` window, "me" = gh accounts / `-u` / `u`). In my turn / active / waiting / PRs rows the cursor previews the **latest comment** (that is why main shows a comment there); my turn rows say why it is on me: `⟵ @who mentioned me +5d ·`, `commented on my PR`, `replied after me` | make it the **current item** — Item, Links, Comments and People follow |
