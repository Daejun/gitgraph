...
| `review_cmd` | `GITGRAPH_REVIEW_CMD` | (empty) | review mode: a slash command to review with instead of gg's own protocol (claude only). One value applies everywhere; `repo-glob=cmd` pairs separated by commas apply per repo, and an entry with no glob is the default |
...
| `ime_switch` | `GITGRAPH_IME_SWITCH` | `true` | TUI: with fcitx5 on this machine, switch the IME to English whenever gg takes the keyboard (start, back from `C`, after a text prompt) and restore it at exit. Nothing runs without a local `fcitx5-remote` (ssh, PuTTY) |
...
| 3 Inbox | one section at a time (`[` `]`): todo (only this session's repos — plus marks on items this graph references; `gg todo` still prints every repo's) · my turn · mentions · opened · active · waiting · mine · PRs by others · stale · all — same rules as before ("my turn" = items I am in where someone else spoke last, `--days N` window, "me" = gh accounts / `-u` / `u`). In my turn / active / waiting / PRs rows the cursor previews the **latest comment** (that is why main shows a comment there); my turn rows end with why it is on me: `⟵ @who mentioned +5d`, `@who on my PR +14d`, `@who replied +2d` | make it the **current item** — Item, Links, Comments and People follow |
...
| `C` | open Claude Code next to gg (a tmux pane, else full screen). Through the `gg mcp` server it sees what you look at (`gg_state`, `gg_context`), your marks (`gg_todo`) and can drive gg (`gg_open`, `gg_mark`) and tick off marks it has handled (`gg_todo_done`) — the running tui drops them at once, also when `gg todo done` ran in another shell. Register once: `claude mcp add -s user gg -- gg mcp` |
...
| Hangul IME | with fcitx5 on this machine gg puts the IME to English itself when it takes the keyboard — at start, back from `C`, when a text prompt closes (a prompt typed in Hangul reopens in Hangul) — and restores it at exit (`ime_switch`). Elsewhere shortcuts still work in Hangul mode, one key late, because the IME holds the letter until the next key: the jamo/syllable is mapped back to the 2-set layout keys (`ㅓ` = j, `ㅏ` = k, `자` = w k) |
...
Then every finding is checked **in a call of its own that tries to disprove it**. The kernel prompt sets tell the reviewer to audit itself in the same session; splitting it out is the point, because a fresh context cannot be dragged along by the reasoning that produced the claim. That call is told to read the code at the line first (reviewers hallucinate line numbers), then argue the author's side — is there really a path where invalid data arrives, does a caller already hold that lock, was ownership handed over, is the surrounding code already written this way — and only then answer itself with code. It comes back `✓` CONFIRMED, `?` PLAUSIBLE or dropped; a dropped one is remembered by content, so a later review of the same PR does not raise it again. `review_verify off` turns the pass off and leaves everything PLAUSIBLE.

`review_cmd` swaps that first pass for a slash command of your own — `/kreview`, sashiko's `/review-pr`, `/code-review` — either everywhere or per repo (`torvalds/linux=/kreview, other/*=/review-pr, /code-review`). The command is handed the range (`<merge-base>..<head>`) and the diff, and gg's output contract is still appended, which is what keeps a borrowed skill's findings drawable in the same panel and postable to the same lines. `claude -p` inherits your MCP servers and settings, so in a kernel tree a skill that expects semcode still has it. A skill often writes its report to a file before answering, so a `review_cmd` run may write inside the throwaway worktree; gg's own protocol stays read-only. If the command is not installed, or simply ignores the contract, gg retries once with its own protocol and says so. The check pass is never delegated — disproving a finding is gg's own discipline.

Remarks about style and design are held to the same discipline as the kernel prompts hold them to: at most three per pull request, and hidden entirely while a confirmed defect is still open (`review_subjective`).

A push does not invalidate a review, only the part the new commits touched. Open the PR again and gg says `reviewed at abc1234 · 2 files changed since — R`, keeping every finding on a file the push did not touch with its verdict intact, re-anchored onto the new diff; `R` then offers to re-read just those two files (`yes — 2 of 9 files changed since abc1234, 6 findings kept`). It gives that up and reviews everything whenever carrying would be a guess: the branch was rebased so every line moved, the head it was reviewed at is no longer in your clone, or nothing survives anyway. The head gg reviewed is kept as a `refs/gg/…-prev` ref so git cannot prune it out from under the comparison.

While it runs, the bottom line shows what the model is actually doing — `Read gc.c · 14 tool calls` — instead of a frozen counter; a review is an agent session of a few dozen tool calls, and watching them pass is the difference between waiting and wondering.
